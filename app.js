App = {
  ...
  render: function() {
    ...
    App.contracts.FakeNews.deployed().then(function(instance) {
      ...
      return newsInstance.newsCount( { from: App.account });
      ...
      newsInstance.isOwnerContract( { from: App.account }).then((isOwner)=> {
        isOwner ?  $("#incentivize").show() :  $("#incentivize").hide() 
        console.log(isOwner)
  ...
  incentivize: function() {

    // e.preventDefault()
    var newsId = $('#newsSelect').val();
    App.contracts.FakeNews.deployed().then(function(instance) {
      return instance.incentivizeVoter(newsId, { from: App.account });
    }).then(function(result) {
      // Wait for votes to update
      $("#content").hide();
      $("#loader").show();
